...
def validate_url(url: str) -> str:
    ...
    # URLs copied from prose often carry a trailing comma or period.  Strip
    # only common sentence punctuation; URL-encoded punctuation remains intact.
...
def _fetch_url(url: str) -> str:
    """Use Naver's public mobile article endpoint when a desktop blog URL is supplied.

    The desktop blog shell contains only an iframe.  Its public mobile endpoint
    serves the same post in one HTML document, which lets the normal local
    extractor read the article without browser clipboard or selection actions.
    """
    parts = urlsplit(url)
...
def webpage_to_pdf(
    ...
    *,
    ...
    # Keep the URL the user supplied in the optional PDF source line while
    # resolving all relative images against the final page URL above.
